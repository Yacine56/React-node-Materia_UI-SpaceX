...
data.map(index=>{
   ...
   { return(
          <Grid key={index.flight_number} item container className={classes.card} direction="row" onClick={()=>{cardClick(index.flight_number)}}>
      <Grid item container direction="column" md className={classes.first}>
      <Typography className={classes.titles}>flight_number : </Typography>
      ...
      {index.details && <Typography className={classes.text}>{index.details} </Typography>}          
       ...
       <Grid item md>
     <img src={index.links.flickr_images[1]} alt="" style={{width:"100%",height:"300px"}}/>
        </Grid>
