...
const FlightDetails = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        let query = {
          "Flight Number": response.data.flight_number,
          "Mission Name": response.data.mission_name,
          "Launch date": response.data.launch_date_utc.split("T")[0],
          "Launch Status": response.data.launch_success ? "SUCCESS" : "FAILED",
          rocket: response.data.rocket.rocket_name,
          "rocket Type": response.data.rocket.rocket_type,
  ...
  let content = loading ? (
    <CircularProgress size="150px" style={{ color: "white" }} />
  ) : (
    <React.Fragment>
        ...
        {images.map((url, index) => {
          return (
            <img
              ...
              style={{ border: selected === url && "3px solid green" }}
            />
          );
        })}
      </Grid>
